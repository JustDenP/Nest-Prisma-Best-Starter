...
export class UserController {
  ...
  findById(@Param() paramID: ParamID) {
    return this.userService.findFirst({ where: { id: paramID.id } });
  }

  @ApiOperation({ summary: 'Update user' })
  @Patch(':id')
  update(@Param() paramID: ParamID, @Body() inputData: UpdateUserDTO) {
    return this.userService.update({ where: { id: paramID.id }, data: inputData });
  }

  @ApiOperation({ summary: 'Soft delete user' })
  @Delete(':id')
  softDelete(@Param() paramID: ParamID) {
    return this.userService.softDelete({ where: { id: paramID.id } });
  }

  @ApiOperation({ summary: 'Restore user' })
  @Patch('/restore/:id')
  restore(@Param() paramID: ParamID) {
    return this.userService.restore({ where: { id: paramID.id } });
  }
  ...
  delete(@Param() paramID: ParamID) {
    return this.userService.permanentDelete({ where: { id: paramID.id } });
  }
}
